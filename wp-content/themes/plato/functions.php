...
function plato_styles() {
	...
	wp_enqueue_style('eltema', PLATO_CSS_PATH.'customizado.css', '', '', 'all');
...
function plato_comment(
	...
	<?php
	printf(__('%1$s at %2$s', 'plato'), get_comment_date(), get_comment_time());
	?></a><?php edit_comment_link(__('(Edit)', 'plato'), '  ', '');
	?>
	</div>
...
function plato_sidebars() {
	...
	/**== Right sidebarInicio ==**/
	register_sidebar(array(
			'name'        => __('SidebarInicio', 'plato'),
			'id'          => 'sidebarInicio',
			'description' => __('This is the main sidebar for inicio.',
				'plato'),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3 class="widget-title">',
...
function plato_social_list() {
	...
	$html .= '<li><a id = "social-'.$sa.'" href="'.esc_url(get_theme_mod($sa.'-url', '')).'"><i class="fa fa-'.$sa.'"></i></a></li>';
	endif;
	endforeach;
