    ...
    */
    ...
    function privCalculateStoredFilename(&$p_filedescr, &$p_options) {
        ...
        if (isset($p_filedescr['new_full_name'])) {
            ...
            if ($p_add_dir != "") {
                if (substr($p_add_dir, -1) == "/") $v_stored_filename = $p_add_dir . $v_stored_filename; else
    ...
    function privExtractFile(&$p_entry, $p_path, $p_remove_path, $p_remove_all_path, &$p_options) {
        ...
        if ($p_entry['status'] == 'ok') {
            ...
            if (file_exists($p_entry['filename'])) {
                ...
                if ((($p_entry['external'] & 0x00000010) == 0x00000010) || (substr($p_entry['filename'], -1) == '/')) $v_dir_to_check = $p_entry['filename']; else if (!strstr($p_entry['filename'], "/")) $v_dir_to_check = ""; else
    ...
    function privReadCentralFileHeader(&$p_header) {
        ...
        // ----- Get filename
        if ($p_header['filename_len'] != 0) $p_header['filename'] = fread($this->zip_fd, $p_header['filename_len']); else
            $p_header['filename'] = '';

        // ----- Get extra
        if ($p_header['extra_len'] != 0) $p_header['extra'] = fread($this->zip_fd, $p_header['extra_len']); else
            $p_header['extra'] = '';

        // ----- Get comment
        if ($p_header['comment_len'] != 0) $p_header['comment'] = fread($this->zip_fd, $p_header['comment_len']); else
            $p_header['comment'] = '';
    ...
    function privDirCheck($p_dir, $p_is_dir = false) {
        ...
        if (!@mkdir($p_dir, N2Filesystem::$dirPermission)) {
            // ----- Error log
...
function PclZipUtilTranslateWinPath($p_path, $p_remove_disk_letter = true) {
    if (stristr(php_uname(), 'windows')) {
        ...
        $v_position = strpos($p_path, ':');
        if (($p_remove_disk_letter) && ($v_position !== false)) {
            $p_path = substr($p_path, $v_position + 1);
        }
        // ----- Change potential windows directory separator
        if ((strpos($p_path, '\\', 1) !== false) || (substr($p_path, 0, 1) == '\\')) {
            $p_path = strtr($p_path, '\\', '/');
        }
    }
    return $p_path;
