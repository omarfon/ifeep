	...
	$('#course_curriculum').on('active',function(){
		$('#add-h5p').on('click',function () {
	        ...
	        $.each(wplms_h5p_contents,function(k,v){
	        	table += '<tr><td>'+v.title+'</td><td><button class="button small wplms-h5p-insert" data-id="'+v.id+'" data-slug="new-h5p-update">'+default_wplms_h5p_strings.insert+'</button></td></tr>';
	        });
	      ...
	      return false;
  	 	...
  	 	$('body').on('click','.wplms-h5p-insert',function () {
	        ...
	        var win = window.dialogArguments || opener || parent || top; 
		 			win.send_to_editor('[h5p id="' + contentId + '"]');
